...
use crate::utils::area_map::area_map::{AreaMap, Position, FromChar};
...
pub fn solution() {
    ...
    let paths: Vec<Position> = vec![Position::new(1, 1), Position::new(3, 1), Position::new(5, 1), Position::new(7, 1), Position::new(1, 2)];

    println!("Day 3 - part 1: {:?}", count_trees_on_path(&mut area_map, &Position::new(3, 1)));
    println!("Day 3 - part 2: {:?}", paths.iter().fold(1, |acc, position| acc * count_trees_on_path(&mut area_map, position)));
}
