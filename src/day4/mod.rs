...
pub fn solution1() {
    ...
    for line in input.lines() {
        if line.len() > 0 {
            let fields: Vec<&str> = line
                .split_whitespace()
                .filter_map(|kv| Some(kv.split(":").collect::<Vec<&str>>()[0]))
                .collect();
            remain_to_check = remain_to_check
                .iter()
    ...
    print_solution(4, 1, total_valid);
}
...
pub fn solution2() {
    ...
    print_solution(4, 2, total_valid);
}
